# Implement step 2, higher temp of https://github.com/ClimateInequality/PrjCEC/issues/37
...
spn_path <- file.path(spt_path_res, "data_tab_a1.csv", fsep = .Platform$file.sep)
df_data_tab_a1 <- read_csv(spn_path)
# latex file name for storage
spn_tex_out <- file.path(spt_path_res, "data_tab_a1.tex", fsep = .Platform$file.sep)
...
df_data_tab_a1 <- df_data_tab_a1 %>%
  select(
    loc_level, region_prov_name,
    contains("32"), contains("35"), contains("38")
  )
...
df_data_tab_a1_prc <- df_data_tab_a1 %>%
  mutate(region_group_label = case_when(
    region_prov_name %in% ls_region_prov$Regional ~ "Regions",
    region_prov_name %in% ls_region_prov$Central ~ "Central",
    region_prov_name %in% ls_region_prov$Eastern ~ "Eastern",
    region_prov_name %in% ls_region_prov$Northeastern ~ "Northeastern",
    region_prov_name %in% ls_region_prov$Western ~ "Western"
  )) %>%
...
if (verbose) {
  print(glue::glue("F-873270, S1"))
  print(df_data_tab_a1_prc)
}

# 3. Count by group
df_data_tab_a1_prc <- df_data_tab_a1_prc %>%
  arrange(region_group_sorter, prov_group_sorter)
# Count by group
df_group_counts <- df_data_tab_a1_prc %>%
...
if (verbose) {
  print(glue::glue("F-873270, S2"))
  print(df_group_counts)
}

# 4. Format columns, decimals, percentage signs, etc
df_data_tab_a1_format <- df_data_tab_a1_prc %>%
  select(
...
# Replace NAs
df_data_tab_a1_format <- df_data_tab_a1_format %>%
...
for (st_kableformat in ar_st_kableformat) {
  ...
  bk_tab_a1 <- bk_tab_a1 %>%
    ...
    column_spec(c(2,3,6,7,10,11), width = "0.60cm") %>%
    column_spec(c(4,5,8,9,12,13), width = "0.80cm")
  # Final adjustments
  # Headings on all pages, note use `sub` to replace first midrule
  st_headend <- paste0(
    "\\midrule\\endhead\n",
    "\\addlinespace[0.2em]\\hline\\addlinespace[0.2em]\n",
    "\\multicolumn{", dim(df_data_tab_a1_format)[2], "}{r}{\\emph{Continued on next page}}\\\\\n",
    "\\endfoot\\endlastfoot"
  )
  bk_tab_a1 <- sub(bk_tab_a1,
    pattern = "\\midrule", replacement = st_headend,
    fixed = TRUE
  )
  # country-names left-align
  bk_tab_a1 <- gsub(bk_tab_a1,
