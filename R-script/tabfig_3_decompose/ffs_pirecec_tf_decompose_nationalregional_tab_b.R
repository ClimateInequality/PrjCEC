...
# central and western
# Copied from `R-script/tabfig_3_decompose/ffs_pirecec_tf_decompose_nationalregional_tab_a.R`, with region selection change
...
spn_tex_out <- file.path(spt_path_res, "tab_b_regional_centralwestern.tex", fsep = .Platform$file.sep)
...
df_data_tab_a_prc <- df_data_tab_a %>%
  mutate(region_name_sorter = case_when(
      region_name == "Central region" ~ "1Central",
      region_name == "Western region" ~ "2Western",
      TRUE ~ region_name
  )) %>%
  filter(
      region_name %in% c("Central region", "Western region"),
...
for (st_kableformat in ar_st_kableformat) {
    ...
    bk_tab_a <- kbl(
        ...
        label = "app:decompose:regionalwesterncentral",
        # escape = F,
        linesep = "",
        booktabs = T,
        longtable = T,
