# 2026-03-15 05:28:11, Note: 
# We conduct a decomposition analysis to consider 1990 demographics jointly with
# 2019-2021 UTCI temperature data in 
# `R-script/run_3a_decompose/ffs_pirecec_demo_1990pop_2019t2021utci_par.R`.
# This script is meant to generate the UTCI temperature input for for that analysis. 
# However, after Fan and Kai discussing on 3/13/2026, we determined that we can just use
# the already generated `df_era5_utci_china_2019_2021_hour.csv` file, there is no need
# to generate a new temperature file. In fact, the output from this script does not
# seem to be working, when used as input in running `ffs_pirecec_demo_1990pop_2019t2021utci_par.R`, 
# it leads to heat exposure results that seem to miss the northeastern region, and have 
# wrong population proportions among remaining regions, indicating errors below that led to 
# possible misalignment of locations or other issues.
# 
# The original purpose of this script seems to be the following: the county shapefile coverages
# in 1990 demographic file and 2020 demographic files are different. We delete some of the rows (coordinates) from
# the full 2019-2021 UTCI coordinate hourly based file to have a perhaps smaller file than the raw file that is appropriate
# and that just includes the coordinate need for 1990 demographics. So if this file worked, 
# it might be slightly faster to load than the full file, but it should generate the same results as the full all
# coordinate temperature file. However, the output from this script leads to a completely different heat
# exposure result file compared to the full file, the full file results are sensible, when compared to the 
# non-decomposition analysis results.
