...
# `R-script/tabfig_2_decompose/ffs_pirecec_tf_region_name_prov_tab_b.R`
...
spn_path <- file.path(spt_path_res, "tab_b_region_data.csv", fsep = .Platform$file.sep)
df_data_tab_b <- read_csv(spn_path)
# latex file name for storage
spn_tex_out <- file.path(spt_path_res, "tab_b_region.tex", fsep = .Platform$file.sep)
...
df_data_tab_b %>% distinct(region_name)
df_data_tab_b_prc <- df_data_tab_b %>%
    filter(
        # region_name %in% c("Central", "Eastern", "Northeastern"),
        region_name %in% c("Eastern", "Northeastern"),
...
df_data_tab_b_sorted <- df_data_tab_b_prc %>%
    arrange(
        region_name, desc(utci_stress_grp_sorter), desc(utci_thres),
    ) %>%
    group_by(utci_stress_grp_sorter) %>%
    ungroup()
# Count by group 1 x group 2
df_group_counts <- df_data_tab_b_sorted %>%
    group_by(region_name, utci_stress_grp, utci_stress_grp_sorter) %>%
    summarize(group_count = n()) %>%
    arrange(region_name, desc(utci_stress_grp_sorter)) %>%
    ungroup() %>%
...
df_data_tab_b_format <- df_data_tab_b_sorted %>%
    select(-region_name, -utci_stress_grp_sorter, -utci_stress_grp) %>%
    mutate_at(
...
for (st_kableformat in ar_st_kableformat) {
    ...
    st_caption <- paste(
        "Decompose by regions changes",
        "in average heat exposure for children (ages 0--14)"
    )
    bk_tab_b <- kbl(
        df_data_tab_b_format,
        format = st_kableformat,
        label = "app:decompose:regional",
        # escape = F,
        linesep = "",
        booktabs = T,
        longtable = T,
