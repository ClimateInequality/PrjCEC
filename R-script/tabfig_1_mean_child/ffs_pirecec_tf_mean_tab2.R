...
spn_path <- file.path(spt_path_res, "data_tab_b.csv", fsep = .Platform$file.sep)
df_data_tab_b <- read_csv(spn_path)
# latex file name for storage
spn_tex_out <- file.path(spt_path_res, "data_tab_b.tex", fsep = .Platform$file.sep)
...
for (n in seq(1, length(numbers))) {
  print(n)
  print(formatC(signif(numbers[n], digits = 3)))
