...
spn_path <- file.path(spt_path_res, "tab_a_24vsday_data.csv", fsep = .Platform$file.sep)
df_data_tab_a <- read_csv(spn_path)
# latex file name for storage
spn_tex_out <- file.path(spt_path_res, "tab_a_24vsday.tex", fsep = .Platform$file.sep)
...
for (st_kableformat in ar_st_kableformat) {
  ...
  # st_title <- paste(
  #   "Changes in average share of time exposed to heat",
  #   "for children (0--14)")
  st_title <- paste(
    "Changes in average share of time exposed to heat"
    )
  bk_tab_a <- kbl(
    ...
    label = "app:meanchild:24hvsdaytime",
    # escape = F,
    linesep = "",
    booktabs = T,
    longtable = T,
  ...
  bk_tab_a <- bk_tab_a %>%
    ...
    column_spec(2:dim(df_data_tab_a_format)[2], width = "1.2cm")
  # Final adjustments
